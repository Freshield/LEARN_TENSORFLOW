...
W1 = tf.Variable(tf.truncated_normal([4, 5000], stddev=1.0))
b1 = tf.Variable(tf.zeros([5000]))
hidden1 = tf.nn.relu(tf.matmul(x, W1) + b1)

W2 = tf.Variable(tf.truncated_normal([5000, 3000], stddev=1.0))
...
for step in range(max_step):
    ...
    if step % 100 == 0:
        ...
        print 'need %f minute' % (use_time * (max_step - step) / 60)


    if step % 1000 == 0:
        data = get_batch_data(test_dataset, batch)
