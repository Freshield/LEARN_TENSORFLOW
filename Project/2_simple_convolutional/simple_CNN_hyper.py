            ...
            with tf.Graph().as_default():
                with tf.Session() as sess:
                    ...
                    cross_entropy = tf.reduce_mean(
                        tf.nn.softmax_cross_entropy_with_logits(labels=labels_one_hot, logits=y_conv),
                        name='xentropy')
                    ...
                    for step in range(max_step):
                        ...
                        last_index, data, out_of_dataset = sequence_get_data(train_dataset, indexs, last_index,
                                                                             batch_size)
                        ...
                        if step % 500 == 0 or step == max_step - 1:
                            result = do_eval(sess, train_dataset, batch_size, correct_num, placeholders, merged,
                                             test_writer, False,
                                             step)
                    ...
                    result = do_eval(sess, test_dataset, batch_size, correct_num, placeholders, merged, test_writer,
                                     False,
                                     step)
