...
def batchnorm_backward_alt(dout, cache):
  ...
  dx = gamma * (N * dout * denorm - dbeta * denorm - norm * dgamma * denorm) / N
...
def max_pool_backward_naive(dout, cache):
  ...
  for n in xrange(N):
    for c in xrange(C):
      for hh in xrange(H_OUT):
        for ww in xrange(W_OUT):
          hstart = hh * stride
          wstart = ww * stride
          window = x[n, c, hstart:hstart + pool_height,wstart:wstart + pool_width]
          out = np.max(window)

          dx[n, c, hstart:hstart+pool_height, wstart:wstart+pool_width] += \
            dout[n, c, hh, ww] * (window == out)
...
def spatial_batchnorm_backward(dout, cache):
  ...
  dx = (1. / Nt) * gamma * (var + eps) ** (-1. / 2.) * (Nt * dout - np.sum(dout, axis=(0,2,3)).reshape(1, C, 1, 1) - (x - mu) * (var + eps) ** (-1.0) * np.sum(dout * (x - mu), axis=(0,2,3)).reshape(1, C, 1, 1))
