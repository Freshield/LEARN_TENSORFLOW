...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    ############################################################################
    afre_out, cache1 = affine_relu_forward(X, self.params['W1'], self.params['b1'])
    scores, cache2 = affine_forward(afre_out, self.params['W2'], self.params['b2'])
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    loss = loss_without_reg + 0.5 * self.reg * np.sum(self.params['W1'] **2) + 0.5 * self.reg * np.sum(self.params['W2'] ** 2)
    daffre_out, grads['W2'], grads['b2'] = affine_backward(dscores, cache2)
    dx, grads['W1'], grads['b1'] = affine_relu_backward(daffre_out, cache1)
  ...
  def loss(self, X, y=None):
    ...
    for i in xrange(1, self.num_layers):
      if not self.use_batchnorm:
        ...
        current_input, bn_net_caches[i] = affine_bn_relu_forward(current_input, self.params['W'+str(i)], self.params['b'+str(i)], self.params['gamma' + str(i)], self.params['beta' + str(i)], self.bn_params[i - 1])

      if self.use_dropout:
        current_input,dropout_cache[i]  = dropout_forward(current_input, self.dropout_param)

    #last layer
    last_out, last_cache = affine_forward(current_input, self.params['W'+str(self.num_layers)], self.params['b'+str(self.num_layers)])
    ...
    #last layer
    last_dx, last_dW, last_db = affine_backward(dscores, last_cache)
    ...
    for i in xrange(self.num_layers-1, 0, -1):
      if self.use_dropout:
        affine_dx = dropout_backward(last_dx, dropout_cache[i])
        last_dx = affine_dx
      ...
      if not self.use_batchnorm:
        ...
        affine_dx, affine_dW, affine_db, dgamma, dbeta = affine_bn_relu_backward(last_dx, bn_net_caches[i])
...
def affine_bn_relu_forward(x, w, b, gamma, beta, bn_param):
    ...
    af_out, fc_cache = affine_forward(x, w, b)
