...
# build graph
def get_conv_layer(images, conv1_depth, conv2_depth, conv3_depth, fc1_size, fc2_size, keep_prob):
    ...
    conv_parameters = (W_conv1, b_conv1, W_conv2, b_conv2, W_conv3, b_conv3, W_fc1, b_fc1, W_fc2, b_fc2)

    return h_fc2_drop, conv_parameters


def get_span_fc_layer(fc2_size, h_fc2_drop, labels_one_hot):
    ...
    fc_parameter = (W_fc3, b_fc3)

    return cross_entropy, fc_parameter, y_conv

def loss_op(conv_parameter, fc_span_parameter, cross_entropy_span, reg):
    W_conv1, b_conv1, W_conv2, b_conv2, W_conv3, b_conv3, W_fc1, b_fc1, W_fc2, b_fc2= conv_parameter

    conv_loss = tf.nn.l2_loss(W_conv1) + tf.nn.l2_loss(b_conv1) + tf.nn.l2_loss(W_conv2) + tf.nn.l2_loss(
            b_conv2) + tf.nn.l2_loss(W_conv3) + tf.nn.l2_loss(b_conv3)+ tf.nn.l2_loss(W_fc1) + tf.nn.l2_loss(
            b_fc1) + tf.nn.l2_loss(W_fc2) + tf.nn.l2_loss(b_fc2)

    fc_loss_total = 0.0
    cross_entropy_total = 0.0

    for i in range(20):
        W_fc3, b_fc3 = fc_span_parameter['span'+str(i)]
        cross_entropy = cross_entropy_span['span'+str(i)]
        fc_loss = tf.nn.l2_loss(W_fc3) + tf.nn.l2_loss(b_fc3)
        fc_loss_total += fc_loss
        cross_entropy_total += cross_entropy

    reg_loss = 0.5 * reg * (conv_loss + fc_loss_total)

    #loss = cross_entropy_total
    loss = cross_entropy_total + reg_loss

    return loss

def predict(y_conv, labels_one_hot):
...
def do_eval(sess, data_set, batch_size, correct_num, placeholders, merged, test_writer, if_summary, global_step):
    ...
    if reset_data_size != 0:
        ...
        count += num
    return count / data_set.shape[0] / 20
...
with tf.Graph().as_default():
    with tf.Session() as sess:
        ...
        h_fc2_drop, conv_parameters = get_conv_layer(images, conv1_depth, conv2_depth, conv3_depth, fc1_size, fc2_size,
                                                keep_prob)
        ...
        for i in range(20):
            ...
            cross_entropy, fc_parameter, y_conv = get_span_fc_layer(fc2_size, h_fc2_drop, labels_one_hot)
            fc_span_parameter['span'+str(i)] = fc_parameter
            cross_entropy_span['span'+str(i)] = cross_entropy
            y_conv_span['span'+str(i)] = y_conv

        loss = loss_op(conv_parameters, fc_span_parameter, cross_entropy_span, reg)
        ...
        for step in range(max_step):
            ...
            if step % 50 == 0:
                print 'loss for in step %d is %f' % (step, loss_v)

            if step % 100 == 0 or step == max_step - 1:
                result = do_eval(sess, train_dataset, batch_size, total_num, placeholders, merged, test_writer,
                                 False,
                                 step)
                print '----------training acc for in step %d is %f-------------' % (step, result)
                result = do_eval(sess, validation_dataset, batch_size, total_num, placeholders, merged,
                                 test_writer,
                                 True, step)
                print '----------validation acc in step %d is %f-------------' % (step, result)
        ...
        result = do_eval(sess, test_dataset, batch_size, total_num, placeholders, merged, test_writer, False,
                         step)
